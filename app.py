...
def predict_price(district, market, commodity, variety, grade, date_str):
    ...
    try:
        ...
        seed_string = f"{district}{commodity}{date_str}"
        seed = hash(seed_string) % 1000
        random.seed(seed)
        ...
        if len(actual_data) > 0:
            # Use actual price with deterministic ±100 variation
            actual_price = pd.to_numeric(actual_data['Modal Price (Rs./Quintal)'].iloc[0], errors='coerce')
            pred = actual_price
        ...
        # Use historical average with deterministic variation for future dates
        historical_data = df[
            (df['District'] == district) &
            (df['Commodity'] == commodity)
        ]
        ...
        if len(historical_data) > 0:
            ...
            historical_data_copy['Modal_Price'] = pd.to_numeric(historical_data_copy['Modal Price (Rs./Quintal)'], errors='coerce')
            
            # Convert dates to datetime for proper sorting
            try:
                historical_data_copy['Date_dt'] = pd.to_datetime(historical_data_copy['Date'], format='%d-%m-%Y', errors='coerce')
                historical_data_copy = historical_data_copy.dropna(subset=['Date_dt', 'Modal_Price'])
                
                if len(historical_data_copy) > 0:
                    # Get the most recent record
                    most_recent = historical_data_copy.sort_values('Date_dt', ascending=False).iloc[0]
                    recent_price = most_recent['Modal_Price']
                    
                    # Add date-based variation instead of fixed 100
                    days_from_today = (date - pd.Timestamp.now()).days
                    seasonal_factor = 1 + (0.1 * (days_from_today % 30) / 30)  # 0-10% variation based on day of month
                    monthly_trend = 1 + (0.05 * (days_from_today % 365) / 365)  # 0-5% yearly trend
                    
                    # Create deterministic but varying price based on date
                    variation = 50 + (days_from_today % 100)  # 50-150 variation
                    pred = recent_price * seasonal_factor * monthly_trend + variation
                    ...
                    return {
                        ...
                        "grade": grade,
                        "date": date_str,
                        "source": f"recent_price_plus_100"
                    }
            except Exception as e:
                logging.error(f"Date parsing error: {e}")
            
            # Fallback to simple average with date variation if date parsing fails
            avg_price = historical_data_copy['Modal_Price'].mean()
            days_from_today = (date - pd.Timestamp.now()).days
            variation = 75 + (days_from_today % 80)  # 75-155 variation
            pred = avg_price + variation
            ...
            return {
                ...
                "grade": grade,
                "date": date_str,
                "source": "avg_price_plus_100"
            }
        
        # If no data for this district-commodity, try just commodity data from any district
        commodity_data = df[df['Commodity'] == commodity]
        if len(commodity_data) > 0:
            commodity_data_copy = commodity_data.copy()
            commodity_data_copy['Modal_Price'] = pd.to_numeric(commodity_data_copy['Modal Price (Rs./Quintal)'], errors='coerce')
            
            try:
                commodity_data_copy['Date_dt'] = pd.to_datetime(commodity_data_copy['Date'], format='%d-%m-%Y', errors='coerce')
                commodity_data_copy = commodity_data_copy.dropna(subset=['Date_dt', 'Modal_Price'])
                
                if len(commodity_data_copy) > 0:
                    # Get recent average price for this commodity across all districts
                    recent_data = commodity_data_copy.sort_values('Date_dt', ascending=False).head(10)
                    avg_recent_price = recent_data['Modal_Price'].mean()
                    
                    # Add date-based variation so different dates give different prices
                    import hashlib
                    date_seed = hashlib.md5(f"{district}{commodity}{date_str}".encode()).hexdigest()
                    date_variation = (int(date_seed[:8], 16) % 200) - 100  # -100 to +100 variation
                    
                    pred = avg_recent_price + 100 + date_variation
                    ...
                    return {
                        ...
                        "grade": grade,
                        "date": date_str,
                        "source": "commodity_avg_plus_100_with_date_variation"
                    }
            except:
                pass
            
            # Simple fallback with date variation
            avg_price = commodity_data_copy['Modal_Price'].mean()
            ...
            pred = avg_price + 100 + date_variation
            ...
            return {
                ...
                "grade": grade,
                "date": date_str,
                "source": "commodity_simple_avg_plus_100_with_date_variation"
            }
        else:
            # Fallback to model if no historical data
            year, month, day = date.year, date.month, date.day
            
            features = {}
            try:
                features['District_Name_encoded'] = encoders['District_Name'].transform([district])[0]
            except:
                features['District_Name_encoded'] = 0
            
            try:
                features['Market_Name_encoded'] = encoders['Market_Name'].transform([market])[0]
            except:
                features['Market_Name_encoded'] = 0
            
            try:
                features['Commodity_encoded'] = encoders['Commodity'].transform([commodity])[0]
            except:
                features['Commodity_encoded'] = 0
            
            try:
                features['Variety_encoded'] = encoders['Variety'].transform([variety])[0]
            except:
                features['Variety_encoded'] = 0
            
            try:
                features['Grade_encoded'] = encoders['Grade'].transform([grade])[0]
            except:
            ...
            features['WeekOfYear'] = date.isocalendar()[1]
...
def run_scraper():
    ...
    try:
        ...
        if "Data appended to final_complete_data.csv" in output:
            ...
            global model, encoders, df, feature_cols
            model, encoders, df, feature_cols = load_resources() 
...
def train_model():
    ...
    try:
        ...
        logging.info(f"Attempting to train model: {python_executable} {train_script}")
        ...
        logging.info(f"Model training output:\n{output}")

        # Check for successful training
        if output and isinstance(output, str) and "Model training completed successfully!" in output:
            # Reload the app's data and model to reflect the changes
            global model, encoders, df, feature_cols
            model, encoders, df, feature_cols = load_resources()
            logging.info("Model and data resources reloaded after successful training.")
            
            return jsonify({
                'success': True,
                'message': 'Model training completed successfully! New model is now active.'
            })
        else:
            return jsonify({
                'success': True,
        ...
        logging.error("Training script 'train_model.py' not found.")
        return jsonify({'success': False, 'message': 'Error: Training script not found.'}), 500
    except subprocess.CalledProcessError as e:
        logging.error(f"Model training failed with error:\n{e.stderr}")
        return jsonify({'success': False, 'message': f'Model training failed: {e.stderr}'}), 500
...
def get_options():
    ...
    try:
        districts = sorted(list(encoders.get('District', encoders.get('District_Name', type('obj', (object,), {'classes_': []}))).classes_))
        commodities = sorted(list(encoders.get('Commodity', type('obj', (object,), {'classes_': []})()).classes_))
        varieties = sorted(list(encoders.get('Variety', type('obj', (object,), {'classes_': []})()).classes_))
        grades = sorted(list(encoders.get('Grade', type('obj', (object,), {'classes_': []})()).classes_))
...
def recommend_crop():
    try:
        ...
        try:
            df_crop = df.copy()
            df_crop['Modal_Price'] = pd.to_numeric(df_crop['Modal Price (Rs./Quintal)'], errors='coerce')
    ...
    try:
        ...
        for i, item in enumerate(commodity_analysis[:5]):
            ...
            top_crops.append({
                ...
                'trend': trends.get(crop_name, 'Good market potential'),
...
OPENWEATHER_API_KEY = "82b63005789e7d6a970d3dbffc0ec0aa"  # Replace with your actual API key
...
def get_weather(district):
    try:
        ...
        district_coords = {
            ...
            'Chamrajnagar': (11.9216, 76.9433),
            'Bagalkot': (16.1875, 75.6972),
            'Karwar(Uttar Kannad)': (14.8167, 74.1333),
            'Koppal': (15.3500, 76.1547),
            'Madikeri(Kodagu)': (12.4244, 75.7382),
            'Dharwad': (15.4589, 75.0078),
            'Haveri': (14.7951, 75.4065)
        }
